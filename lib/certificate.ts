...
export async function generateCertificate(
  ...
  try {
    ...
    const COORDS = {
      ...
      userName: { y: 680, size: 48 },
      // labelProgramKursus: { y: 650, size: 11 },
      courseName: { y: 540, size: 34 },
      ...
      footer: {
        ...
        predicateValue: { x: 135, y: 250, size: 30 },
        dateLabel: { y: 150, size: 11 },
        dateValue: { y: 250, size: 30 },
      },
    };
    ...
    // const dateLabelWidth = fontRegular.widthOfTextAtSize(
    //   dateLabelText,
    //   COORDS.footer.dateLabel.size,
    // );
    const dateValueWidth = fontBold.widthOfTextAtSize(
      issueDateStr,
      COORDS.footer.dateValue.size,
    );
    const dateXBase = width - 430;
